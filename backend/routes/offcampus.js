...
// ==========================================================
// 0️⃣ EXPORT ALL DRIVES — SINGLE PDF
// ==========================================================
router.get('/export-all', async (req, res) => {
  try {
    const drives = await OffCampusDrive.find().sort({ dateOfRecruitment: -1 });

    // Prepare PDF
    const doc = new PDFDocument({ margin: 30, size: 'A4' });

    res.setHeader('Content-Disposition', `attachment; filename=offcampus_all_drives.pdf`);
    res.setHeader('Content-Type', 'application/pdf');

    doc.pipe(res);

    // Title page / header
    doc.fontSize(20).text('Off Campus Recruitment — All Drives Report', { align: 'center' });
    doc.moveDown(0.5);
...
// ==========================================================
// 1️⃣ STUDENT ROUTES — MUST ALWAYS COME FIRST
// ==========================================================
...
// ==========================================================
// 2️⃣ EXPORT PDF ROUTE — single drive
// ==========================================================
router.get('/:id/export', async (req, res) => {
  ...
  try {
    ...
    // HEADER
    doc.fontSize(18).text(`Off-Campus Drive: ${drive.collegeName}`, { underline: true });
    doc.moveDown();
...
router.post('/', async (req, res) => {
  try {
    ...
    const drive = new OffCampusDrive(payload);
    await drive.save();
    res.status(201).json(drive);
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  ...
  const drive = await OffCampusDrive.findByIdAndUpdate(req.params.id, payload, { new: true });
  res.json(drive);
});
...
// add somewhere above the generic /resume/:filename route
router.get('/resume/view/:filename', (req, res) => {
  try {
    console.log('OFFCAMPUS RESUME VIEW hit:', req.params.filename);
    ...
    console.error('View resume error (offcampus):', err);
