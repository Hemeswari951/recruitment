...
class _EmployeeNotificationsPageState extends State<EmployeeNotificationsPage> {
  ...
  Future<void> fetchPerformanceNotifications() async {
    final uri = Uri.parse(
      "https://company-04bz.onrender.com/notifications/performance/employee/$selectedMonth/${widget.empId}?year=$selectedYear",
    );
    final resp = await http.get(uri);
  ...
  Future<void> fetchHolidayNotifications() async {
    final uri = Uri.parse(
      "https://company-04bz.onrender.com/notifications/holiday/employee/${widget.empId}?month=$selectedMonth&year=$selectedYear",
    );
    final resp = await http.get(uri);
  ...
  Widget notificationCard(
    ...
    // Find the other person in conversation
    String otherUserId =
          ...
          child: Column(
            ...
            children: [
              ...
              if (isExpanded && category == "message") ...[
                ...
                // Map through the messages array from the backend
