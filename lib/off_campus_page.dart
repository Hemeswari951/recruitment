...
class OffCampusPage extends StatefulWidget {
  ...
  @override
  State<OffCampusPage> createState() => _OffCampusPageState();
}

class _OffCampusPageState extends State<OffCampusPage> {
  ...
  Future<void> _loadDrives() async {
    ...
    try {
      final data = await OffCampusService.fetchDrives();
      setState(() {
        _drives = data
  ...
  Future<void> _createOrUpdateDrive(
    ...
    try {
      if (id == null) {
        await OffCampusService.createDrive(payload);
      } else {
        await OffCampusService.updateDrive(id, payload);
      }
      await _loadDrives();
    } catch (e) {
  ...
  Future<void> _confirmDelete(String id) async {
    ...
    if (ok == true) {
      try {
        await OffCampusService.deleteDrive(id);
        await _loadDrives();
        ScaffoldMessenger.of(
          context,
  ...
  /// Export SINGLE Drive PDF (Works on Web + Mobile + Desktop)
  Future<void> _exportPdfAndOpen(
    String driveId, {
    String? suggestedName,
  }) async {
    final url = '${OffCampusService.baseUrl}/api/offcampus/$driveId/export';
    ...
    if (kIsWeb) {
       html.AnchorElement(href: url)
        ..setAttribute(
          'download',
          suggestedName ?? 'offcampus_drive_$driveId.pdf',
        )
        ..click();
      return;
    }
    ...
    // ⭐ MOBILE + DESKTOP
    try {
      final resp = await OffCampusService.exportDrivePdf(driveId);
      ...
      if (resp.statusCode == 200) {
        ...
        final fileName =
            suggestedName ??
            'offcampus_drive_${driveId}_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  Future<void> _downloadAllDrivesPdf() async {
    final url = '${OffCampusService.baseUrl}/api/offcampus/export-all';
    try {
      if (kIsWeb) {
        // Trigger direct download via anchor
         html.AnchorElement(href: url)
          ..setAttribute('download', 'offcampus_all_drives.pdf')
          ..click();
        ...
        if (resp.statusCode == 200) {
          ...
          final filePath =
              '${baseDir.path}/offcampus_all_drives_${DateTime.now().millisecondsSinceEpoch}.pdf';
